...
namespace MyRainbow
{
	...
	{
		static string GetConnectionStringFromSecret(string[] args, string configParam = "SqlConnection")
		{
		...
		internal static void RedisExample(string[] args)
		{
			...
			using (var redis = new RedisHasher(GetConnectionStringFromSecret(args, "Redis")))
			{
				redis.EnsureExist();
				redis.Purge();
		...
		internal static void MongoDBExample(string[] args)
		{
			...
			using (var dbase = new MongoDBHasher(GetConnectionStringFromSecret(args, "MongoDB")))
			{
				dbase.EnsureExist();
				//dbase.Purge();
